  ...
  import { ListView, Flex, Card, Toast, Checkbox, SwipeAction, List } from 'antd-mobile';
    ...
    getContent() {
      ...
      .done(( res ) => {
        ...
        if ( this.props.editbill.filter !== '' ) {
          list = res.data.items.filter(({ dmid,housecode }) => (dmid === this.props.editbill.filter)||(housecode === this.props.editbill.filter) );
        } else {
          list = res.data.items;
        }
